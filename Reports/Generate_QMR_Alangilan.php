...
<title>Generate QMR - Alangilan</title>
...
<?php }}?>
	...
	<tr>
		<td colspan="13"> <b>College of Engineering</b></td>
	</tr>
<?php
...
$sql = ("SELECT * FROM create_alangilan WHERE 
		(Year = $Year) AND
		(Quarter = $Q) AND
		(Office LIKE '%COE%' OR Office LIKE '%College of Engineering%') AND
		(Remarks = 'Evaluated')");
$command = $con->query($sql) or die("Error finding Offices under CIT");
//$No = 0;
while($result = mysqli_fetch_array($command))
...
<?php }}?>
	...
	<table class="signatories">
		...
		<tr>
			...
			<div class="Getbtn">
				<button id = "BtnReceived" onclick="Received()"> ✓ </button>
			</div>	
